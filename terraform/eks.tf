...
# Allow nodes to reach the private API endpoint
...
resource "aws_eks_cluster" "main" {
  ...
  vpc_config {
    ...
    endpoint_public_access  = false   # API only reachable from within the VPC
  }

  # Encrypt secrets at rest using AWS-managed key (no extra cost)
  encryption_config {
    resources = ["secrets"]
    provider {
...
# ── Managed Node Group (SPOT + ON_DEMAND mix for cost savings) ────────────────
...
resource "aws_eks_node_group" "main" {
  ...
  # Use SPOT to cut EC2 costs by ~60-70%
  capacity_type = "SPOT"
